...
function preload () {
    imgMask = loadImage("https://img.icons8.com/officel/2x/dead-face.png");
    imgFace = loadImage("https://img.icons8.com/officel/2x/scared-face-meme.png");
}
...
function drawMask () {
    ...
    if (positions !== false) {
        push();
        const wx = Math.abs(positions[13][0] - positions[16][0]) * 5;
        const wy = Math.abs(positions[7][0] - Math.min(positions[16][0], positions[20][0])) * 5;
        translate(-wx/2 - wy/2);
        image(imgMask, positions[62][0], positions[62][0], wx, wy);
        pop();
    }
}
